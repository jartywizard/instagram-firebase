...
//  InstagramFirebase
//
//  Created by Joseph Clinch on 5/26/18.
...
class UserProfileHeader: UICollectionViewCell {
    ...
    @objc func handleEditProfileOrFollow() {
        ...
        //The following creates a new node in the Firebase Database for followers
        
        guard let currentLoggedInUserId = Auth.auth().currentUser?.uid else { return }
        
        guard let userId = user?.uid else { return }
        
        //This below checks if the button is currently "Unfollow" and then tells it to behave accordingly
